...
foreach ($guide in @('CHARACTER_PROFILE.md', 'FICTIONAL_HISTORY.md')) { Copy-Item -LiteralPath (Join-Path $projectRoot "docs/$guide") -Destination (Join-Path $portable $guide) }
Compress-Archive -LiteralPath (Join-Path $portable 'Novel-Studio.exe'), (Join-Path $portable '使用說明.md'), (Join-Path $portable '更新紀錄.md'), (Join-Path $portable 'CHARACTER_PROFILE.md'), (Join-Path $portable 'FICTIONAL_HISTORY.md') -DestinationPath (Join-Path $destination "Novel-Studio-$Version-Windows-x64-NoInstall.zip") -Force
